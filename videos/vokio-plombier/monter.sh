...
if [ "$1" = "--rendre" ]; then
  echo "· rendu"
  # Le nom de sortie suit le métier déclaré dans le storyboard : une déclinaison
  # ne doit pas écraser le MP4 du film de référence.
  M=$(sed -n 's/^metier: *//p' STORYBOARD.md | head -1)
  OUT="renders/vokio-${M:-promo}-9x16.mp4"
  hf render --skill=product-launch-video --quality looks --output "$OUT"
